...
func clearJobsDb(t *testing.T, db *gorm.DB) {
	err := db.Exec(`TRUNCATE s, pipeline_runs, pipeline_specs, pipeline_task_runs, pipeline_task_specs CASCADE`).Error
...
func TestORM_CreateRunWhenJobDeleted(t *testing.T) {
	...
	require.EqualError(t, err, "no  found with id -1 (most likely it was deleted)")
}
